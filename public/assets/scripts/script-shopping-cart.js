  ...
  var items = new Array();
    ...
    function addItemsToHtmlShopping(){
      ...
      $.each(items, function(index, value){
        var prod = JSON.parse(items.getItem(index));
        var produc;

        let getProductRequest = getProductsRequest + prod.id;

        $.getJSON( getProductRequest, function( data ) {
          produc = JSON.parse(data);
        }).done(function(){
          lstProductsSorted.push({"id": produc.id, "name" : produc.name, "price": produc.price, "quantity": prod.quantity});
        });
      });
    ...
    function calculTotalPrice(){
      ...
      $.each(lstProductsSorted, function(index, value){
        var product = JSON.parse(lstProductsSorted.getItem(index));
    ...
    function calculTotalQuantity(){
      var totalQuantity = 0;
      $.each(lstProductsSorted, function(index, value){
        let product = JSON.parse(lstProductsSorted.getItem(index));
        totalQuantity = parseInt(totalQuantity) + parseInt(product.quantity);
      });
